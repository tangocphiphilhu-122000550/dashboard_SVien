...
const Login = ({ onLoginSuccess }) => {
  ...
  /*
  ================================================================================
    CODE CŨ (Bắt buộc nhập đúng Email/MSSV & Mật khẩu để gửi Backend API/DB)
  ================================================================================
  ...
  */
  ...
  // --- CODE MỚI DÙNG DEMO (Hỗ trợ tự điền tài khoản demo nếu để trống) ---
  const handleLogin = async () => {
    const loginId = identifier.trim() || 'demo@student.edu.vn';
    const loginPass = password || '123456';

    const { user } = await loginWithCredentials({ identifier: loginId, password: loginPass });
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[linear-gradient(135deg,#f8fbff_0%,#edf4ff_34%,#dceafe_100%)] px-4 py-10 text-slate-900">
      ...
      <section className="glass-panel relative z-10 mx-auto w-full max-w-md p-6 sm:p-8">
        ...
        <div className="mt-6 text-center">
          ...
          <div className="mt-4 rounded-2xl border border-blue-200/80 bg-blue-50/80 p-3 text-xs leading-5 text-blue-800">
            💡 <strong>Chế độ Demo (Offline)</strong>: Ứng dụng chạy hoàn toàn ở phía Frontend, không cần kết nối Backend/Database. Bấm <strong>Đăng nhập</strong> trực tiếp để bắt đầu trải nghiệm.
          </div>
        </div>
